...
/// numpy.isclose semantics: |a - b| <= atol + rtol * |b|
#[inline]
fn is_close_python(a: f64, b: f64, atol: f64, rtol: f64) -> bool {
...
///
/// Boundary handling mirrors Python's reference: when an old bin edge sits
/// outside `new_bins` due to floating-point precision, Python asserts
/// `np.isclose(atol=5e-8, rtol=1e-5)` between the offending edge and the
/// nearest cluster edge and proceeds. A real out-of-range input is a data bug
/// and Python crashes; this implementation returns a typed error to the
/// Python wrapper instead.
///
/// Returns a Vec<f32> of length `new_bins.len() - 1`.
fn rebin_histogram(values: &[f32], old_bins: &[f64], new_bins: &[f64]) -> Result<Vec<f32>, String> {
    const ATOL: f64 = 5.0e-8;
    const RTOL: f64 = 1.0e-5;

    let n_new = new_bins.len() - 1;
    let mut out = vec![0f32; n_new];
    let nb_first = new_bins[0];
    let nb_last  = new_bins[n_new];

    for (i, &val) in values.iter().enumerate() {
        if val == 0.0f32 {
            continue;
        }
        let mut lo = old_bins[i];
        let mut hi = old_bins[i + 1];
        let width = hi - lo;
        if width <= 0.0 {
            continue;
        }
        ...
        // = last edge index where edge <= lo. Returns -1 (saturated to 0) when
        //   lo < new_bins[0] — Python asserts close-to-boundary and snaps lo
        //   to new_bins[0]; we mirror that.
        let pp_lo = new_bins.partition_point(|&x| x <= lo);
        let start = if pp_lo == 0 {
            if !is_close_python(lo, nb_first, ATOL, RTOL) {
                return Err(format!(
                    "old_bin lower edge {} below cluster range start {} (not close enough to snap)",
                    lo, nb_first
                ));
            }
            lo = nb_first;
        ...
        // Python: when this returns len(new_bins), assert close to right edge
        //   and snap hi to new_bins[-1].
        let pp_hi = new_bins.partition_point(|&x| x < hi);
        let end = if pp_hi >= new_bins.len() {
            if !is_close_python(hi, nb_last, ATOL, RTOL) {
                return Err(format!(
                    "old_bin upper edge {} above cluster range end {} (not close enough to snap)",
                    hi, nb_last
                ));
            }
            hi = nb_last;
        ...
        // Recompute width if we snapped boundaries, so the fraction below stays consistent.
        let width = hi - lo;
        if width <= 0.0 {
            continue;
        }
...
pub fn build_rebinning_index_cluster<'py>(
    ...
    let rebinned_result: Result<Vec<Vec<f32>>, String> = py.allow_threads(|| {
        (0..n_hists)
            .into_par_iter()
            .map(|i| rebin_histogram(&vals_data[i], &bins_data[i], &new_bins_vec))
            .collect()
    });
    let rebinned = rebinned_result
    ...
    //
    // Byte-match Python's reference build:
    //   - Python computes `np.cumsum(rebinned_hists[i][1], axis=1)` where the
    //     input array is f32, so cumsum accumulates in f32.
    //   - Python then rounds via `np.round(arr, 4)` which uses banker's rounding
    //     (round half to even).
    // We must use an f32 accumulator and `round_ties_even` for the stored values
    // to be bit-identical to Python's index. The previous f64 accumulator with
    // `round()` (round-half-away-from-zero) produced indices that differed from
    // Python's by ≤1 ULP and could flip individual query results on values
    // that rounded differently between the two strategies.
    let round_factor: f32 = 10f32.powi(rounding_precision);
    ...
    for h in 0..n_hists {
        ...
        for k in 0..row.len() {
            cum += row[k];
            let rounded = (cum * round_factor).round_ties_even() / round_factor;
            // Fortran index for (h, k+1): (k+1)*n_hists + h
            pctls_flat[(k + 1) * n_hists + h] = rounded;
