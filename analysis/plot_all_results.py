...
def fig21_pin_cores():
    """Pin-cores flattens the speedup curve at high thread counts.

    Shows pooled+f16 with and without --features pin-cores. Median of 3 runs.
    Data captured 2026-04-27 in same session as fig20.
    """
    threads      = [1, 2, 4, 8, 16, 32, 64]
    no_pin       = [13.34, 7.99, 4.45, 3.27, 2.86, 4.00, 3.65]   # pooled+f16-native medians
    with_pin     = [14.33, 8.39, 4.82, 3.13, 2.82, 3.09, 3.16]   # pooled+f16+pin medians
    python       = 18.27
    ...
    # Left: absolute time
    ax1.axhline(python, color=PYTHON_COL, linestyle="--", lw=1.4,
                label=f"Python single-thread ({python:.2f}s)", alpha=0.8)
    ax1.plot(threads, no_pin,   color=RUST_COL, marker="o", lw=1.8, ms=6,
             label="pooled+f16 (no pin)")
    ax1.plot(threads, with_pin, color=GREEN, marker="s", lw=1.8, ms=6,
             label="pooled+f16 + pin-cores")
    ax1.annotate(
        f"t=32: 4.00s → 3.09s (+23%)",
        xy=(32, with_pin[5]),
        xytext=(8, 12),
        fontsize=8.5, color=GREEN,
        arrowprops=dict(arrowstyle="->", color=GREEN, lw=1.0),
    ...
    ax1.set_title("Pin-cores flattens the curve at $t \\geq 32$")
    ax1.legend(loc="upper right")
    ax1.set_ylim(0, 18)

    # Right: speedup over Python
    sp_no  = [python/x for x in no_pin]
    sp_pin = [python/x for x in with_pin]
    ax2.axhline(1.0, color="gray", lw=1.0, linestyle=":", alpha=0.5,
                label="Python baseline (1×)")
    ax2.plot(threads, sp_no,  color=RUST_COL, marker="o", lw=1.8, ms=6,
             label="pooled+f16")
    ax2.plot(threads, sp_pin, color=GREEN, marker="s", lw=1.8, ms=6,
             label="pooled+f16 + pin-cores")
    ax2.set_xscale("log", base=2)
    ax2.set_xticks(threads)
    ax2.get_xaxis().set_major_formatter(ticker.ScalarFormatter())
    ax2.set_xlabel("Rayon threads")
    ax2.set_ylabel("Speedup over Python single-thread")
    ax2.set_title("Speedup curve")
    ax2.legend(loc="lower right")
    ax2.set_ylim(0, 7)

    fig.suptitle(
        "Phase 17 — Physical-core pinning eliminates SMT-sibling co-location\\\\\n"
        "Speedup stays within 10\\% of peak across $t \\in [16, 64]$ instead of dropping sharply",
        fontsize=10.5, y=1.04,
    )
    fig.tight_layout()
    save(fig, "fig21_pin_cores")


def fig20_pooled():
    """Pre-allocated per-query output Vec — biggest single-thread win.

    All measured 2026-04-26, eval_medium, suppress_results=True.
    Default + pooled-tuned in same session (bcvs3frj9).
    Pooled-tuned + f16 in immediately-following session (b8ozi2zgv).
    """
    threads     = [1, 2, 4, 8, 16, 32, 64]
    default     = [25.17, 9.06, 4.36, 3.29, 3.25, 3.48, 3.53]
    pooled      = [13.92, 8.21, 4.77, 4.13, 3.05, 3.31, 3.56]
    pooled_f16  = [13.34, 7.99, 4.45, 3.27, 2.89, 3.16, 3.24]
    python      = 18.27
    ...
    ax1.axhline(python, color=PYTHON_COL, linestyle="--", lw=1.4,
                label=f"Python single-thread ({python:.2f}s)", alpha=0.8)
    ax1.plot(threads, default,    color=RUST_COL, marker="o", lw=1.8, ms=6,
             label="Rust default (flat_map+collect)")
    ax1.plot(threads, pooled,     color=GREEN, marker="s", lw=1.8, ms=6,
             label="Rust + pooled output (--features pooled)")
    ax1.plot(threads, pooled_f16, color="#762a83", marker="^", lw=1.8, ms=6,
             label="Rust + pooled + f16 (best config)")

    # Annotate Python-cross at t=1
    ax1.annotate(
        f"Rust beats Python at t=1\n(1.37× faster)",
        xy=(1, pooled_f16[0]),
        xytext=(2.3, 22),
    ...
    ax1.set_xlabel("Rayon threads")
    ax1.set_ylabel("Query time (s)")
    ax1.set_title("Pre-allocated per-query output Vec — eval\\_medium")
    ax1.legend(loc="upper right")
    ax1.set_ylim(0, 28)

    # Right panel: speedup over Python
    speedup_default = [python/x for x in default]
    speedup_pooled  = [python/x for x in pooled]
    speedup_pf16    = [python/x for x in pooled_f16]
    ax2.axhline(1.0, color="gray", lw=1.0, linestyle=":", alpha=0.5,
                label="Python baseline (1×)")
    ax2.plot(threads, speedup_default, color=RUST_COL, marker="o", lw=1.8, ms=6,
             label="Default")
    ax2.plot(threads, speedup_pooled,  color=GREEN, marker="s", lw=1.8, ms=6,
             label="Pooled")
    ax2.plot(threads, speedup_pf16,    color="#762a83", marker="^", lw=1.8, ms=6,
             label="Pooled + f16")
    ax2.set_xscale("log", base=2)
    ax2.set_xticks(threads)
    ax2.get_xaxis().set_major_formatter(ticker.ScalarFormatter())
    ax2.set_xlabel("Rayon threads")
    ax2.set_ylabel("Speedup over Python single-thread")
    ax2.set_title("Speedup over Python")
    ax2.legend(loc="upper right")
    ax2.set_ylim(0, 7)

    fig.suptitle(
        "Pre-allocated output Vec eliminates 30\\% of cycles previously spent in FlatMap::next\\\\\n"
        "Pooled+f16 wins at every thread count (peak 6.32× at t=16); Rust beats Python even at t=1",
        fontsize=10.5, y=1.04,
    )
    fig.tight_layout()
    save(fig, "fig20_pooled")
