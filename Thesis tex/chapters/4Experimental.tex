...
\begin{table}[H]
  \centering
  \caption[Python baseline wall-clock.]{Python baseline wall-clock, median of 5 reps, \texttt{with-results} mode, 10K queries. The \texttt{c256\_56gb} cell exceeds the per-run cap, its wall is measured via a 1K-query subsample scaled $\times 10$ (§\ref{sec:exp:e2e:mat}).}
...
\begin{figure}[H]
  ...
  \caption[Search-phase wall time and the four ceiling regimes.]{Search-phase wall time of the default (SoA + f32) build on \texttt{c256\_56gb} across thread counts. Both axes are log-scaled. Shaded areas signify the regimes in which each ceiling binds: (i) single-thread compute / L1 latency at $t \leq 8$, (ii) shared L3 capacity at $t \in [8, 32]$, (iii) on-socket memory-traffic pressure at $t \in [32, 48]$, and (iv) cross-socket UPI interconnect at $t > 48$. Error bars show $95\%$ CIs from the 5-repetition measurements.}
...
\begin{table}[H]
  \centering
  \caption[\texttt{simd} vs.\ default.]{\texttt{simd} vs default, suppress mode, median of 5. Ratio $<$ 1 means \texttt{simd} faster.}
  \label{tab:simd}
  \begin{tabular}{lrrrrrr}
    \toprule
    Dataset & $t{=}1$ & $t{=}8$ & $t{=}16$ & $t{=}32$ & $t{=}64$ & $t{=}96$ \\
...
\begin{table}[H]
  \centering
  \caption[\texttt{batch-search} vs.\ columnar default.]{\texttt{batch-search} vs \texttt{columnar-default}, suppress mode, median of 5. Ratio $<$ 1 means \texttt{batch-search} faster.}
...
\begin{table}[H]
  \centering
  \caption[\texttt{horizontal-simd} vs.\ columnar default.]{\texttt{horizontal-simd} vs \texttt{columnar-default}, suppress mode, median of 5. Ratio $<$ 1 means \texttt{horizontal-simd} faster.}
  \label{tab:horizsimd}
  \begin{tabular}{lrrrrrr}
    \toprule
    Dataset & $t{=}1$ & $t{=}8$ & $t{=}16$ & $t{=}32$ & $t{=}64$ & $t{=}96$ \\
...
\begin{table}[H]
  \centering
  \caption[\texttt{pgm} vs.\ default.]{\texttt{pgm} vs \texttt{default} (row-centric), suppress mode, median of 5. Ratio $<$ 1 means \texttt{pgm} faster.}
  \label{tab:pgm}
  \begin{tabular}{lrrrrrr}
    \toprule
    Dataset & $t{=}1$ & $t{=}8$ & $t{=}16$ & $t{=}32$ & $t{=}64$ & $t{=}96$ \\
...
Counting hot-loops for each cluster on the default build with $t=1$ show the dependent-load chain dominates cycle budget. IPC is $1.80$ on \texttt{c256\_10gb}, $1.93$ on \texttt{c256\_30gb}, $1.87$ on \texttt{c256\_56gb}, below the retirement ceiling and points to a load-latency-bound loop. The L1d miss-per-load rate at $t{=}1$ is $\sim\!0.49$--$0.68\%$ for all three datasets. Neither the misprediction rate or the L1d-miss rate exceed the noise floor.

That means that the wall time is dominated by the number of dependent-load chains, one per cluster, instead of the depth or throughput of any single chain. The four ablations shorten or pipeline individual chains, none of them reduces the count, which is why they all measure null.

Ceiling~(i) is part of the ceilings due to it being the most commonly assumed ceiling in literature on in-memory binary-search workloads. Kim et al.'s FAST index~\cite{kim_2010} co-optimises for cache-line, page, and SIMD width on the assumption that L1-latency and CMOV-chain length bind search cost, Khuong and Morin~\cite{khuong_morin_2017} benchmark array layouts under the same assumption, Ferragina and Vinciguerra's PGM~\cite{ferragina_2020} addresses the $\log_2 n$ dependent-load chain as its whole motivation, and Schlegel et al.'s SIMD $k$-ary search~\cite{schlegel_2009} treats the same chain as the cost to be shortened.

Our falsification of ceiling~(i) is what enables us to explore the other ceilings and gives us the opportunity to set aside the single-thread compute assumption and reason about the other ceilings. This negative result is positive evidence about binding constraints.
...
\begin{table}[H]
  \centering
  \caption[\texttt{aos} vs.\ default.]{\texttt{aos} vs default, suppress mode, median of 5 reps. Ratio $>$ 1 means \texttt{aos} slower, ratio $<$ 1 means \texttt{aos} \emph{faster} (bold cells).}
  \label{tab:aos}
  \begin{tabular}{lrrrrrr}
    \toprule
    Dataset & $t{=}1$ & $t{=}8$ & $t{=}16$ & $t{=}32$ & $t{=}64$ & $t{=}96$ \\
...
\begin{figure}[H]
  ...
  \caption[AoS wall-time delta across threads.]{AoS layout wall-time delta versus default (SoA + f32) on \texttt{c256\_56gb} across threads. Red bars: AoS slower. Green bars: AoS faster. Error bars show $95\%$ CIs propagated through the delta computation.}
...
\begin{figure}[H]
  ...
  \caption[\texttt{f16} fine-$t$ wall-time delta.]{\texttt{f16} vs.\ default wall-time delta on \texttt{c256\_56gb}. Saturated bars mark cells where $t$ $95\%$ CI (dof $= n_a + n_b - 2$, §\ref{sec:method:ci}) excludes zero, pale bars are noise-band.}
...
\begin{table}[H]
  \centering
  \caption[\texttt{pooled} vs.\ default.]{\texttt{pooled} vs default, suppress mode. Ratio $<1$ means \texttt{pooled} faster.}
  \label{tab:pooled}
  \begin{tabular}{lrrrrrr}
    \toprule
    Dataset & $t{=}1$ & $t{=}8$ & $t{=}16$ & $t{=}32$ & $t{=}64$ & $t{=}96$ \\
...
\begin{table}[H]
  \centering
  \caption[\bcombo{}-minus-\texttt{pooled} isolation.]{\bcombo{}-minus-\texttt{pooled} vs full \bcombo{}. Positive removal cost means \texttt{pooled} was helping the bundle, negative means \texttt{pooled} was \emph{hurting} it.}
...
At high $t$ on the larger datasets, aggregate memory traffic puts the per-socket memory subsystem under pressure. We observe \emph{anti-scaling}: wall-clock rises while cores are added. Optimisations that could improve this ceiling include reducing bytes-per-emit or bytes-per-comparison.
...
\begin{table}[H]
  \centering
  \caption[\texttt{packed-ids} vs.\ default.]{\texttt{packed-ids} vs default, suppress mode, median of 5. Ratio $<$ 1 means \texttt{packed-ids} faster.}
  \label{tab:packed}
  \begin{tabular}{lrrrrrr}
    \toprule
    Dataset & $t{=}1$ & $t{=}8$ & $t{=}16$ & $t{=}32$ & $t{=}64$ & $t{=}96$ \\
...
\begin{table}[H]
  \centering
  \caption[NUMA placement probe.]{NUMA placement probe, \texttt{c256\_56gb}, \texttt{packed-ids} build, suppress mode, median of 5. Wall in seconds.}
  \label{tab:numa}
  \begin{tabular}{lrrrr}
    \toprule
...
\begin{table}[H]
  \centering
  \caption[Thread scaling of the default build.]{Default Rust build, suppress mode, median of 5. ``Best/$t{=}1$'' is the speedup at the best thread count vs $t{=}1$.}
...
\begin{table}[H]
  \centering
  \caption[\texttt{cluster-par} vs.\ default.]{\texttt{cluster-par} vs default. Ratio $>$ 1 means \texttt{cluster-par} slower.}
...
\begin{table}[H]
  \centering
  \caption[Query-batch $K$-sensitivity.]{$K$-sensitivity at $t{=}96$ on \texttt{c256\_56gb}, suppress mode, median of 5.}
  \label{tab:qb_k}
  \begin{tabular}{rrrr}
    \toprule
...
\begin{table}[H]
  \centering
  \caption[Morsel vs.\ regime-best builds.]{\texttt{c256\_56gb}, suppress mode, median of 5. Wall in seconds. Best per row in bold.}
  \label{tab:morsel}
  \footnotesize
...
\begin{table}[H]
  \centering
  \caption[The five negative-composability instances.]{The five negative-composability instances. Read each row as: baseline composition plus the finer ablation layered on top of it. The full mechanism stories are developed in §\ref{sec:exp:negcomp:arithmetic} and the section cited per row.}
  \label{tab:negcomp}
  \footnotesize
...
\begin{table}[H]
  \centering
  \caption[Pre-registered bandwidth-budget arithmetic.]{Pre-registered bandwidth-budget arithmetic for the four variants.}
...
\begin{table}[H]
  \centering
  \caption[Per-cluster reindexing results.]{\texttt{c256\_56gb}, row-centric engine, suppress mode, median of 5 reps.}
...
The miss rate per instruction is $\sim\!52\times$ higher under \texttt{local-ids}, and the wall-time ratio at the same cell is $3.4\times$.  The access pattern per-emit is shifted by the narrower decoder from ${\sim}20$-bit-aligned strides to ${\sim}13$-bit-aligned strides. That crosses the boundaries of the $4$\,KiB pages far more frequently, exhausting the dTLB at the multi-thread cells. TLB-walk stalls drown the bandwidth saving by more than an order of magnitude.
...
The policy picks the empirical regime-best build in $17$ of $18$ cells of the $(n_\text{hists}, n_\text{threads})$ grid. The remaining cell is within $5\%$: a $+4.7\%$ gap on \texttt{c256\_30gb} at $t{=}64$, inside the run-to-run variance band at that thread count.

Because the policy's boundaries are read off the calibration grid itself, this in-distribution record confirms that the boundaries were derived consistently. The validation is the out-of-distribution point in §\ref{sec:exp:ood}, which the policy was never designed for.
...
\begin{table}[H]
  \centering
  \caption[Dispatch validation at the OOD point.]{Dispatch validation at \texttt{c1024\_56gb}. ``Gap'' is the wall-time gap between the policy's pick and the empirical regime-best for that cell.}
...
\begin{table}[H]
  \centering
  \caption[OOD structural-validation verdicts.]{Eight pre-registered claims, eight verdicts after 5-rep 95\% CI checks. Density range tested: ${\sim}8\,200$ to ${\sim}26\,300$ hists/cluster ($3.2\times$).}
...
\begin{table}[H]
  \centering
  \caption[Speedup vs.\ the Python baseline at $t{=}16$.]{Speedup at $t{=}16$ across the three calibration datasets. Rust wall is the \bcombo{} bundle. Python is GIL-bound and flat across thread counts (§\ref{sec:exp:python:wall}). One $t{=}16$ measurement per dataset is therefore representative of any $t$. All walls are 5-rep medians.}
  \label{tab:e2e}
  \footnotesize
...
\begin{figure}[H]
  ...
  \caption[End-to-end and search-phase speedup over Python.]{Rust \bcombo{} speedup over the Python baseline at $t=16$ across the three calibration workloads, End-to-end (with-results) stays in the $530$--$598\times$ range because of Python's per-\emph{(query, cluster)} result-dict construction. Search phase alone (suppress) drops monotonically from $22.7\times$ $\to$ $1.34\times$. Log $y$-axis.}
  \label{fig:exp:e2e_speedup}
\end{figure}

\section{Rust Rebinning Kernel\label{sec:exp:rebinning}}

We fuse the two stages of the Python rebinning (per-histogram rebinning under \texttt{multiprocessing.Pool}, then \texttt{numpy.cumsum} + \texttt{argsort(axis=0)} per cluster) into a single fused Rust call per cluster. The kernel parallelises the rebinning across all cores, followed by in-place cumsum and a stable \texttt{sort\_by}.

\begin{table}[H]
  \centering
  \caption[Rebinning kernel wall times.]{Rebinning kernel wall across the three calibration datasets, median of 3 reps.}
  \label{tab:rebin}
  \footnotesize
  \begin{tabular}{lrrr}
    \toprule
...
\begin{table}[H]
  \centering
  \caption[Hypotheses and their verdicts.]{Hypotheses from Chapter~\ref{cha:chapter3} and their verdicts.}
...
Four ceilings bind Fainder's percentile workload on this platform: single-thread compute / L1 latency, shared L3 capacity, on-socket memory-traffic pressure, and the cross-socket UPI interconnect. Latency optimisations (SIMD, batch-search, horizontal-SIMD, PGM) all measured null. Bandwidth optimisations (\texttt{f16}, \texttt{packed-ids}, \texttt{pooled}, \texttt{mimalloc}) and single-socket placement each moved the wall on their own ceiling.

The composition of these optimisations is negative. Five instances on five hardware axes are the proof, alongside their respective \texttt{perf} counters. The ceiling that binds at the composition is \emph{not} the one the second ablation tries to address. This leads to the pre-flight methodology, where we probe the binding ceiling at the target composition before committing to an implementation.

The dispatch policy is valid in-distribution ($17/18$ exact, $18/18$ within $5\%$) and out-of-distribution ($6/6$ within $5\%$ at $3.2\times$ sparser clusters). Ceilings (i) and (iv) reproduce across density while ceiling (ii) has a density-dependent threshold. Four of the five negative-composability instances reproduce at the OOD point, and morsel scheduling attenuates to within-noise.

The Rust port that carried these measurements delivers $530$--$598\times$ end-to-end over the Python baseline at $t=16$, and $114\times$ at the only paired single-thread cell. Most of that ratio is Python's per-\emph{(query, cluster)} result materialisation ($96.9$--$99.8\%$ of its wall, §\ref{sec:exp:e2e:mat}). The search phase itself scales $22.7\times$, $4.6\times$, $1.34\times$ as the dataset grows. NUMA placement adds $11$--$13\%$ at $t \leq 48$, and the rebinning kernel a separate $\sim\!4.7\times$, dropping to $\sim\!2.2\times$ at five million histograms (§\ref{sec:exp:rebinning}).
