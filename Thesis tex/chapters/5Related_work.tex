...
Work done by Manegold et al.~\cite{manegold_2000} illustrates that main-memory access is the dominant bottleneck for in-memory database operations, from that work cache-conscious design principles were establish, which subsequently influenced much of the work in database systems. Manegold et al.'s analysis showed that column-major (SoA) storage dramatically outperforms row-major (AoS) storage for projective (columnar) access patterns, which is the direct motivation for the memory layout designed in Chapter~\ref{cha:chapter3}.

Adaptive Radix Trees (ART)~\cite{leis_2013} adapt their node fanout to the key distribution, achieving cache efficiency for point and range queries. They avoid fixed-fanout waste of classical B-trees. ARTs target key-value loops rather than percentile predicate evaluations, nontheless their design philosophy of having a data structure based on cache line behaviour rather than asymptotic complexity informs the approach taken in this thesis.
...
A comprehensive analysis by Kersten et al.~\cite{kersten_2018} of vectorised group-by aggregation showed that SIMD instructions provide 4--8$\times$ speedup for fixed-width arithmetic operations on sorted arrays. We implement explicit AVX-512 SIMD intrinsics for the final stage of Fainder's binary search (Section~\ref{sec:exp:c1:simd}), replacing the last ${\sim}16$ scalar iterations with a single vector comparison.

Polychroniou and Ross~\cite{polychroniou_2014} performed a comprehensive study of hash join algorithms in main-memory databases on multicore CPUs, in which work-stealing schedulers significantly outperform static task partitioning in the case of sub-tasks with unequal cost. Fainder's cluster queries have costs that are heterogeneous (large cluster $\to$ more histograms $\to$ more binary search steps). This leads us to choose Rayon's work stealing scheduler (an implementation of the scheduler of Blumofe and Leiserson~\cite{blumofe_leiserson_1999}) over round-robin thread assignment. Our reports in Chapter~\ref{cha:chapter4} are consistent with Polychroniou and Ross's findings: speedup grows with thread count until the memory-traffic and interconnect ceilings bind (§\ref{sec:exp:scaling}).
...
\textbf{L1-resident sorted-array search.} Fainder's binary search runs entirely against L1-resident data, since the bin-edge array (~142 floats per cluster) and the per-bin cumulative-density column (~1{,}400 floats per query) are both small enough. The literature on cache-conscious sorted-array search is as follow:

\begin{itemize}
    \item Kim et al.'s FAST index~\cite{kim_2010} reports speedups of multiple magnitudes by re-organising search trees to exploit page spize, cache-line size, and SIMD width.
    ...
    \item Schlegel et al.'s k-ary search on modern processors~\cite{schlegel_2009} is the underlying foundation for Eytzinger and k-ary search explored in earlier ablation campaigns of this thesis. It is also the foundation for SIMD k-ary search, which is the final stage of Fainder's binary search (Section~\ref{sec:exp:c1:simd}).
\end{itemize}
...
\begin{itemize}
    ...
    \item No evaluation, to our knowledge, exists of inter-query SIMD for binary-search-shaped workloads at Fainder's cluster-loop scale on Sapphire Rapids; existing evaluations such as Polychroniou et al.\ and Lang et al.~\cite{polychroniou_2014,lang_2020} predate the microarchitecture.
\end{itemize}

This thesis addresses the first gap with Fainder's query pipeline, due to its embarrassingly parallel structure, its access pattern being binary search, and its read-only index. It characterises the workload so that subsequent work can address the second.